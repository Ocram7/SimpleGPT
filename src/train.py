...
def build_optimizers(model_parts, job_config: JobConfig, world_mesh=None):
    ...
    def _build_optimizer(model, world_mesh=None):
        ...
        if name == "Adam":
            ...
            logger.info("======>>>>> Using AdamW optimizer, lr = {lr}")
        else:
...
def validate(job_config, model, data_loader_val, loss_fn, current_step):
    ...
    total_tokens = 0
    num_val_batch = (
        job_config.training.num_val_batch
    )  # limit the number of data for validation
    current_batch_idx = 0
    ...
    logger.info(
        f"Validation completed: step: {current_step}, val loss: {avg_loss} val token: {total_tokens}"
    )
    model.train()
    return avg_loss
...
def main(job_config: JobConfig):
    ...
    print("---*** ***---")
    print("data_parallel_degree: {}".format(job_config.training.data_parallel_degree))
    print(
        "tensor_parallel_degree: {}".format(job_config.training.tensor_parallel_degree)
    )
    print(
        "pipeline_parallel_degree: {}".format(
    ...
    if getattr(job_config.training, "seed", None) is not None:
        final_seed = set_seed(job_config.training.seed, rank=global_rank)
        if global_rank == 0:
            logger.info(
                f"Using base seed {job_config.training.seed}, rank seed {final_seed}"
            )
    ...
    run_id = (
        ...
        + str(job_config.optimizer.lr)
    )
    logger.info(
        f"=========> Currently running: {run_id}  {job_config.metrics.wandb_comment}"
    )
    ...
    print("dp_rank: {}".format(dp_rank))
    print("---*** ***---")

    print("---*** ***---")
    print("model_parts length: {}".format(len(model_parts)))
    print(model_parts)
    print("---*** ***---")
    ...
    metric_logger = build_metric_logger(
        job_config,
        metrics_log_rank=get_metrics_rank(world_mesh, parallel_dims),
        run_id=run_id,
    )
    ...
    if job_config.metrics.enable_wandb and global_rank == 0:
        ...
        run_id = (
            ...
            + job_config.optimizer.name
            + str(job_config.optimizer.lr)
            + job_config.metrics.wandb_comment
        )
    ...
    with maybe_enable_profiling(
        ...
        while train_state.step < job_config.training.steps:
            ...
            if parallel_dims.pp_enabled:
                logger.info(
                    "are we here? pipeline parallel forward / backward inside step() call"
                )  # False
                ...
                for microbatch_idx in range(
                    job_config.training.grad_accumulation_steps
                ):
                    # get batch
                    data_load_start = timer()
                    batch = next(data_iterator)
                    ...
                    with loss_parallel_ctx():
                        ...
                        losses_since_last_log.append(
                            loss_unnormalized
                        )  # need to log the un-normalized loss. It will be normalized later in the log function
